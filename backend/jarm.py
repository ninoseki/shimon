...
def supported_versions(jarm_details: JARM_DETAILS, grease: bool) -> bytes:
    ...
    versions = choose_grease() if grease is True else b""
    for version in tls:
        versions += version
...
async def send_packet(  # noqa: C901
    ...
    raw_ip: bool = False
    ip: tuple[str | None, int | None] = (None, None)
...
def read_packet(data: bytes | None) -> str:
    try:
        if data is None:
            return "|||"
        jarm = ""
...
def extract_extension_info(data: bytes, counter: int, server_hello_length: int) -> str:
    try:
        ...
        if (data[counter + 50 : counter + 53] == b"\x0e\xac\x0b") or (
...
def cipher_bytes(cipher: str) -> str:
    ...
    return "0" + hexvalue if len(hexvalue) < 2 else hexvalue
